...
type Option = { text: string; correct: boolean; audioSrc?: string | null; imageSrc?: string | null };
type Challenge = { type: "SELECT" | "ASSIST"; question: string; level?: number; tag?: string; options: Option[] };
...
function validate(c: Course) {
  ...
  c.units.forEach((u, ui) => u.lessons.forEach((l, li) => l.challenges.forEach((ch, ci) => {
    ...
    if (!["SELECT", "ASSIST"].includes(ch.type)) problems.push(`${where}: type ${ch.type}`);
    if (!ch.question?.trim()) problems.push(`${where}: empty question`);
    const correct = ch.options.filter((o) => o.correct).length;
    if (ch.options.length < 2 || correct !== 1) problems.push(`${where}: ${ch.options.length} options, ${correct} correct`);
  })));
  return problems;
}
...
async function main() {
  ...
  const [c] = await db.insert(schema.courses).values({ title: course.title, imageSrc: course.imageSrc ?? "/kr.svg" }).returning();
  let n = 0;
  for (const [ui, u] of course.units.entries()) {
    ...
    for (const [li, l] of u.lessons.entries()) {
      ...
      for (const [ci, ch] of l.challenges.entries()) {
        const [row] = await db.insert(schema.challenges).values({ lessonId: lesson.id, type: ch.type, question: ch.question, order: ci + 1, level: ch.level ?? null, tag: ch.tag ?? null }).returning();
        await db.insert(schema.challengeOptions).values(ch.options.map((o) => ({ challengeId: row.id, text: o.text, correct: o.correct, audioSrc: o.audioSrc ?? null, imageSrc: o.imageSrc ?? null })));
        n++;
      }
    }
  }
