...
type FooterProps = { onCheck: () => void; status: "correct" | "wrong" | "none" | "completed"; disabled?: boolean; lessonId?: number; wrongHint?: string };

// Verdict row + one full-width button, like Solingo's engine. Green / rose wash on the whole footer.
export const Footer = ({ onCheck, status, disabled, lessonId, wrongHint }: FooterProps) => {
  ...
  return (
    <footer className={cn("border-t-2 px-5 pb-[calc(16px+env(safe-area-inset-bottom))] pt-4 lg:px-10", status === "correct" && "border-transparent bg-green-100", status === "wrong" && "border-transparent bg-rose-100")}>
      <div className="mx-auto flex max-w-[600px] flex-col gap-3">
        ...
        {status === "wrong" && <div className="flex items-center text-xl font-bold text-rose-500"><XCircle className="mr-3 h-7 w-7" />아쉬워요{wrongHint ? <span className="ml-3 text-base font-semibold text-rose-500/80">{wrongHint}</span> : null}</div>}
