...
const ProfilePage = async ({
  ...
  const [ownedRows, streak, couple, report, completedRows, achievements] =
    await Promise.all([
      ...
      db
        ...
        .where(eq(challengeProgress.userId, requested)),
      getAchievements(requested),
    ]);
  const owned = Object.fromEntries(ownedRows.map((r) => [r.itemKey, r.qty]));
  const userRow = await db.query.user.findFirst({
    where: eq(user.id, requested),
    columns: { createdAt: true },
  });
  const createdLabel = userRow?.createdAt
    ...
    <div className="flex flex-row-reverse gap-[48px] px-4 sm:px-6">
      ...
      <FeedWrapper>
        <ProfileView
          ...
          targetName={
            users.find((u) => u.userId === requested)?.userName ??
